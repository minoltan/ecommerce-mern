# Session 15 — Hands-On: Auditing & Hardening the Checkout Saga (Advanced Capstone)

This is a companion to the Session 15 lecture plan in `CURRICULUM.md`. Unlike every prior
session, you're not writing a new module from a blank slate here — this lab walks through
something closer to real production work: **auditing code someone else (or past-you) wrote**,
proving what's actually wrong with a failing test, fixing it, and keeping the docs honest.
Every step below is a real commit in this repo's history — you can `git show <hash>` any of
them to see the literal diff.

## Before You Start

- Sessions 1–14 done: the full backend, tests for all seven modules (Session 10), Docker
  (Session 11), the React client (Sessions 12–13), and the microservices-extraction exercise
  (Session 14) all complete.
- `docs/hld/`, `docs/lld/`, `docs/adr/` exist as empty folders going into this lab (check:
  `ls docs/hld docs/lld docs/adr` — nothing there yet at the start).
